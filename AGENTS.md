...
- 当多个相互引用的仓库需要一起提交时，必须先梳理依赖关系，从最底层 `go.mod` 对应仓库开始逐个提交、逐个打 tag，再按依赖方向逐级向上提交。
- 执行上述逐级提交前，必须先将当前待提交仓库内引用的自有模块版本统一升级到可用的最新版本，再执行测试、提交与打 tag。
- 若用户要求“提交”且涉及多仓库联动，默认按“升级依赖版本 -> 底层仓库提交并打 tag -> 上层仓库升级依赖后继续提交并打 tag”的顺序执行。
